...
async fn main() -> std::io::Result<()> {
    ...
    //create a databse connection pool
    let manager = ConnectionManager::<PgConnection>::new(vars::database_url());
    let pool: models:Pool = r2d2::Pool::builder().build(manager).expect("Failed to create a database connection pool.");
    ...
    .blind(format!("{}:{}", vars:domain(), vars::port()))?
    .run()
    .await
}